...
//    givenThat(anton).wasAbleTo(openTheHomePage);
//    when(anton).attemptsTo(Search.forTheTerm("This search is not working now!"));
//  }
//
  @Test
  public void changeImageLogo() {
    givenThat(anton).attemptsTo(openTheHomePage);
    when(anton).attemptsTo(PressOnLogo.press()); //TODO or when(anton).attemptsTo(Click.on(GuzikchirikGithubioPage.logo())); ---???
